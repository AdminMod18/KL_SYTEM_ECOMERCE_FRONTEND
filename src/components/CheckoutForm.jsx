...
export function CheckoutForm({ items, total, clienteId, onClienteIdChange, onSubmit, loading, error }) {
  if (!items.length) {
    return (
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-5">
      <div className="rounded-2xl border border-border bg-surface p-6 shadow-card lg:col-span-2">
        <h2 className="text-xs font-semibold uppercase tracking-[0.15em] text-text-muted">Summary</h2>
        <ul className="mt-4 space-y-3">
          {items.map((p) => (
            <li key={p.id} className="flex justify-between gap-3 text-sm">
              ...
              <span className="shrink-0 font-medium text-text-primary">{formatMoney(Number(p.precio) * p.cantidad)}</span>
            </li>
          ))}
        </ul>
      ...
      <form onSubmit={onSubmit} className="rounded-2xl border border-border bg-surface p-6 shadow-card lg:col-span-3">
        ...
        <p className="mt-1 text-sm text-text-secondary">
          Sent to <code className="rounded bg-page px-1.5 py-0.5 text-xs text-text-primary">order-service</code> —{' '}
          <code className="rounded bg-page px-1.5 py-0.5 text-xs">POST /orden</code>
        </p>
        {error && (
